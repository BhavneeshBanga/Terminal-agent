...
Claude-Code-style Textual TUI for BhavAI.
...
def _should_skip_file(path: Path) -> bool:
    ...
    if path.suffix.lower() in IGNORE_EXTS:
        return True
    try:
        if path.stat().st_size > 500_000:  # skip anything bigger than ~500KB
            return True
    except OSError:
        return True
    return False
...
def generate_bhavai_md(root: Path) -> Path:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fname in sorted(filenames):
            ...
            truncated_note = "\n... [truncated]\n" if len(text) > MAX_FILE_CHARS else ""
    ...
    header = (
        f"# BHAVAI.md\n\n"
        f"_Auto-generated by `/init` — a full snapshot of this codebase for the "
        f"BhavAI agent to use as context in future sessions._\n\n"
        f"**Root:** `{root}`\n\n"
        f"## Project structure\n\n```\n" + "\n".join(tree_lines) + "\n```\n"
        f"\n## File contents\n"
    )
